...
test_that("tz_lookup_coords2 works", {
  ...
  expect_equal(tz_lookup_coords2(c(70, -70), c(30, -30)),
               c("Europe/Oslo", "Etc/GMT+2"))
  expect_error(tz_lookup_coords2(1, 1:2),
               "lat and lon must numeric vectors be of the same length")
  expect_error(tz_lookup_coords2("a", "b"),
               "lat and lon must numeric vectors be of the same length")
...
test_that("tz_lookup_coords2 deals with NAs", {
  ...
  expect_equal(tz_lookup_coords2(rep(NA_real_, 2), rep(NA_real_, 2)),
               rep(NA_character_, 2))
  expect_equal(tz_lookup_coords2(c(NA_real_, 70), c(NA_real_, 30)),
               c(NA, "Europe/Oslo"))
  expect_equal(tz_lookup_coords2(c(NA_real_, 70), c(1, 30)),
               c(NA, "Europe/Oslo"))
})
...
test_that("tz_lookup2.sf works", {
  ...
  expect_error(tz_lookup_coords2(pts),
  ...
  expect_equal(tz_lookup2(pt, "+proj=aea +lat_1=50 +lat_2=58.5 +lat_0=45 +lon_0=-126 +x_0=1000000 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"), # nolint
               "Etc/GMT+9")
})
